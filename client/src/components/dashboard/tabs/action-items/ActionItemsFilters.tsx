...
export const ActionItemsFilters: React.FC<ActionItemsFiltersProps> = React.memo(({
  ...
  return (
    <div className="p-4 sm:p-5 rounded-2xl bg-white dark:bg-[#0f172a] border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-3.5">
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-50/80 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs font-medium text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-hidden focus:ring-2 focus:ring-[#5cb028]/20 focus:border-[#5cb028] transition-all"
          />
          {searchQuery && (
            <button
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none pl-3 pr-8 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-600 focus:outline-hidden cursor-pointer"
            >
      ...
      <div className="flex items-center gap-1 pt-1 border-t border-slate-100 dark:border-slate-800 overflow-x-auto">
        {[
            ...
            >
              ...
              <span
                className={`px-1.5 py-0.2 rounded-full text-[10px] font-black ${
                  isActive ? 'bg-white/25 text-white' : 'bg-slate-200/70 dark:bg-slate-800 text-slate-600 dark:text-slate-300'
                }`}
              >
                {tab.count}
              </span>
